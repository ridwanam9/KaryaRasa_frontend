...
export default function Sellers() {
  ...
  useEffect(() => {
    ...
    fetch("https://dying-helli-ridwanam9-4b98d171.koyeb.app/users/")
      ...
      .then((data) => {
        // Filter users based on the role "seller"
        const sellerData = data.filter((user: Seller) => user.role === "seller");
  ...
  if (loading) return <p className="text-gray-600 text-sm">Loading sellers...</p>;
  if (error) return <p className="text-red-600 text-sm">{error}</p>;
  if (sellers.length === 0) return <p className="text-gray-600 text-sm">No sellers found.</p>;
  ...
  return (
    <table className="min-w-full border border-gray-300 rounded overflow-hidden">
      ...
      <tbody>
        {sellers.map(({ id, name, email, role, phone, address, bank_account, birth_date }) => (
